...
from myutils.resnet import Resnet18
...
def run_one_epoch(epoch,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if train:
            ...
            # model.module.conv1.weight.retain_grad()
            loss = forward_loss(model,
                                criterion,
                                input,
                                target,
            ...
            weight_grad = model.module.conv1.weight.grad.data
            bias = model.module.conv1.bias.data
            bias_grad = model.module.conv1.bias.grad.data
            first_order_taylor += ((weight*weight_grad).pow(2).view(64,-1).sum(dim=1)+\
                (bias*bias_grad).pow(2))
            counter += 1
            # if counter > 100:
            #     np.save("taylor.npy", first_order_taylor.data.cpu().numpy())
            #     exit()
    val_top1 = None
...
def train_val_test():
    ...
    #     if 'weight' in key:
    #         print(key, checkpoint[key].size())
    # exit()
    equiv_net = Resnet18()
    def get_fixed_net_name(name):
        if name == 'conv1':
            return 'module.head.0'
        if name == 'fc':
            return 'module.classifier.0'
        if 'conv' in name:
            stage = int(name[5]) - 1
            layer = int(name[7])
            body = int(name[-1]) - 1
            return f'module.stage_{stage}_layer_{layer}.body.{body}'
        if 'downsample.' in name:
            stage = int(name[5]) - 1
            layer = int(name[7])
            return f'module.stage_{stage}_layer_{layer}.shortcut.0'
        return None
    def calib_conv_weight(weight, fix_net_name, checkpoint):
        eps = 1e-05
        bn_mean = checkpoint[f'{fix_net_name}.bn.running_mean']
        bn_std = torch.sqrt(checkpoint[f'{fix_net_name}.bn.running_var'] + eps)
        weight = (checkpoint[f'{fix_net_name}.bn.weight'] / bn_std)[:, None, None, None] * weight
        bias = checkpoint[f'{fix_net_name}.bn.bias'] - checkpoint[f'{fix_net_name}.bn.weight'] / bn_std * bn_mean
        return weight.data, bias.data
    for name, module in equiv_net.named_modules():
        fix_net_name = get_fixed_net_name(name)
        if fix_net_name is not None and name != 'fc':
            print(name, fix_net_name)
            weight = checkpoint[f'{fix_net_name}.conv.weight'].data
            assert module.weight.size() == checkpoint[f'{fix_net_name}.conv.weight'].size()
            module.weight.data, module.bias.data = calib_conv_weight(weight, fix_net_name, checkpoint)
        elif name == 'fc':
            module.weight.data = checkpoint[f'{fix_net_name}.weight'].data
            module.bias.data = checkpoint[f'{fix_net_name}.bias'].data
    # exit()
    idx = FLAGS.channel_idx
    weight = equiv_net.conv1.weight.data
    weight[idx] = weight[idx]*15/7
    equiv_net.conv1.weight.data = weight.data

    model_wrapper = torch.nn.DataParallel(equiv_net).cuda()
